...
namespace SweetShopView
{
    public static class MailClient
    {
        ...
        private static void GetMessages(int number)
        {
            ...
            string requestSubjectMessage = string.Empty;
            string requestBodyMessage = string.Empty;
            ...
            while (true)
            {
                ...
                if (response.Length > 4)
                {
                    ...
                    {
                        requestSubjectMessage = GetSubject(ref response, ref coding);

                        requestBodyMessage = GetBody(response, coding);
                    }
                    if (!string.IsNullOrEmpty(messageId) && !string.IsNullOrEmpty(from) &&
                        !string.IsNullOrEmpty(requestSubjectMessage) && !string.IsNullOrEmpty(date))
                    {
                        APICustomer.PostRequestData("api/MessageInfo/AddElement",
                            new MessageInfoBindingModel
                            {
                                ...
                                Subject = requestSubjectMessage,
                                Body = requestBodyMessage
                            });

                        messageId = string.Empty;
                        from = string.Empty;
                        date = string.Empty;
                        requestSubjectMessage = string.Empty;
                        requestBodyMessage = string.Empty;
                    }
                }
            }
        }
